...
if uploaded_file:
    # Excelni o‘qish – header=0 ga e’tibor bering
    df = pd.read_excel(uploaded_file, header=0)  # header=0 – birinchi qator ustun nomlari
    ...
    if missing_cols:
        ...
        st.write("Detected columns in Excel:", df.columns.tolist())
    else:
        ...
        # Pie charts
        st.plotly_chart(px.pie(df_filtered, names='Stage', title='Stage Distribution'))
        st.plotly_chart(px.pie(df_filtered, names='Responsible', title='Responsible Distribution'))
        st.plotly_chart(px.pie(df_filtered, names='Source', title='Source Distribution'))
        ...
        st.plotly_chart(px.line(time_data, x='Creation Date', y='Count', title='Records Created Over Time'))
        ...
        st.plotly_chart(px.line(mod_time_data, x='Modified Date', y='Count', title='Records Modified Over Time'))
        ...
        st.plotly_chart(px.bar(top_companies, x='Company name', y='Records', text='Records', title='Top 10 Companies'))
